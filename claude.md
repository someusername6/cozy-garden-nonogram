...
│   ├── js/
│   │   ├── utils.js        # Shared utilities, creates window.Cozy namespace
...
├── content/                # Source artwork
│   ├── sprites/            # Active puzzle sprites (130 PNG files)
│   ├── archive/            # Old/unused sprites for reference
│   └── prompts.txt         # Retrodiffusion prompts used
├── dist/                   # Production build (generated)
│   ├── index.html          # Transformed HTML
│   ├── js/app.min.js       # Minified JS bundle
...
### Build all puzzles from sprites
```bash
cd tools && python3 build_puzzles.py ../content/sprites --report ../report.txt
```
This outputs puzzle data to `src/data/puzzles.js` (~186KB for 130 puzzles).

### View the game (development)
```bash
open src/index.html
```
...
```bash
npm run build      # Creates minified dist/ folder
npm run preview    # Build and serve at http://localhost:3000
npm run clean      # Remove dist/
```

## Build Pipeline

The production build (`npm run build`) creates an optimized `dist/` folder for deployment:

### What it does
1. **JavaScript**: Concatenates 8 JS files from `src/js/` → minifies with esbuild → generates source map
2. **CSS**: Minifies `src/css/style.css` → generates source map
3. **HTML**: Updates `<link>` and `<script>` tags to reference minified files
4. **Service Worker**: Updates cache version with content hash (triggers cache refresh on code changes)
5. **Assets**: Copies `puzzles.js`, `manifest.json`, and icons as-is

### Size reduction
- JavaScript: 177KB → 67KB (62% reduction)
- CSS: 61KB → 42KB (31% reduction)
- Gzipped transfer: ~60KB total for JS+CSS

### Source maps
Source maps are included for production debugging. The `app.src.js` file contains readable concatenated source with file markers, allowing DevTools to show meaningful stack traces.

### Development vs Production
- **Development**: Open `src/index.html` directly, edit source files in `src/`
- **Production**: Deploy `dist/` folder. Service worker cache auto-updates on code changes.
...
- Light/Dark themes styled as "Day in the Garden" (vibrant greens, sunlit warmth) and "Night in the Garden" (deep navy, moonlit silvers). Defaults to system preference for new users. Theme buttons labeled "Light/Dark" (not "Day/Night") for clarity and accessibility convention.
