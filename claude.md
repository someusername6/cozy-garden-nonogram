...
```
nonogram/
├── index.html          # Web player entry point (development)
├── css/
...
│   ├── index.html      # Transformed HTML (minified references)
│   ├── css/
│   │   ├── style.min.css      # Minified CSS
│   │   └── style.min.css.map  # Source map
│   ├── js/
│   │   ├── app.min.js         # Minified JS bundle (all 8 files)
│   │   ├── app.min.js.map     # Source map
│   │   └── app.src.js         # Readable concatenated source (for debugging)
│   ├── data/
│   │   └── puzzles.js         # Copied as-is (already compact)
│   ├── sw.js           # Service worker with content-hashed cache version
│   └── assets/         # Copied icons and manifest
├── images/
│   └── input/          # Source pixel art sprites (130 PNG files)
├── build.js            # Production build script (esbuild)
├── build_puzzles.py    # Content pipeline (images → puzzle data)
...
### What it does
1. **JavaScript**: Concatenates 8 JS files in dependency order → minifies with esbuild → generates source map
2. **CSS**: Minifies `style.css` → generates source map
3. **HTML**: Updates `<link>` and `<script>` tags to reference minified files, updates preload hints
...
### Source maps
Source maps are included for production debugging. The `app.src.js` file contains readable concatenated source with file markers:
```javascript
// ============================================================
// FILE: js/utils.js
// ============================================================
```
This allows DevTools to show meaningful stack traces and source locations.

### Why puzzles.js isn't minified
The puzzle data is already compact (single-letter keys, numeric arrays). Minification saves only ~100 bytes after gzip - not worth the complexity.

### Development vs Production
- **Development**: Open `index.html` directly, edit source files in `js/` and `css/`
- **Production**: Deploy `dist/` folder. The service worker cache version auto-updates when code changes.
